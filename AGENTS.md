...
The dev toolchain is preinstalled in the VM (not via the startup update script) and its
environment variables are exported from `~/.bashrc`, so a login shell finds them:

- **JDK 21** (Temurin/OpenJDK) — matches the CI `java-version: '21'`.
- **Gradle 8.10.2** at `~/tools/gradle-8.10.2` (on `PATH` as `gradle`). Real projects should
...
- **Android SDK** at `~/android-sdk` (`ANDROID_HOME`/`ANDROID_SDK_ROOT`): `platform-tools`,
  `platforms;android-34`, `build-tools;34.0.0`, plus `cmdline-tools/latest` (`sdkmanager`,
  `avdmanager`, `apkanalyzer`) and `platform-tools` (`adb`) on `PATH`. All SDK licenses are
  accepted.
