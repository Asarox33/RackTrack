...
object MatchEngine {
    ...
    internal val RACK_ENDING_TYPES =
        setOf(
    ...
    fun recordFoul(
        ...
        if (!threeFoulLossApplies) {
            return match.copy(
                ...
                foul2 = nextFoul2,
                pushOutPhase =
                    if (match.pushOutPhase == PushOutPhase.AVAILABLE) {
                        PushOutPhase.NONE
                    } else {
                        match.pushOutPhase
                    },
    ...
    fun canAnnouncePushOut(
        match: Match,
        playerId: PlayerId,
    ): Boolean = PushOutEngine.canAnnounce(match, playerId)

    fun announcePushOut(
        match: Match,
        playerId: PlayerId,
        nowMillis: Long,
    ): Match = PushOutEngine.announce(match, playerId, nowMillis)

    fun resolvePushOutClean(
        match: Match,
        playerId: PlayerId,
        nowMillis: Long,
    ): Match = PushOutEngine.resolveClean(match, playerId, nowMillis)

    fun resolvePushOutFoul(
        match: Match,
        playerId: PlayerId,
        nowMillis: Long,
    ): Match = PushOutEngine.resolveFoul(match, playerId, nowMillis)

    fun takePushOut(
        match: Match,
        nowMillis: Long,
    ): Match = PushOutEngine.take(match, nowMillis)

    fun returnPushOut(
        match: Match,
        nowMillis: Long,
    ): Match = PushOutEngine.giveBack(match, nowMillis)
            ...
            when (last.playerId) {
                match.player1.id ->
                    match.copy(
                        ...
                        runOut1 =
                            if (last.type == MatchEventType.RUN_OUT) {
                                (match.runOut1 - 1).coerceAtLeast(0)
                            } else {
                                match.runOut1
                            },
                        goldenBreak1 =
                            if (last.type == MatchEventType.GOLDEN_BREAK) {
    ...
    private fun undoRackLoss(
        ...
        val withLossCounter =
            if (last.type == MatchEventType.EIGHT_BALL_LOSS) {
                when (last.playerId) {
                    match.player1.id ->
                        revertedScores.copy(
    ...
    private fun nextBreakerAfterRack(
        match: Match,
        rackWinnerId: PlayerId,
    ): PlayerId =
     ...
     */
    private fun breakerFromHistory(
        match: Match,
    ...
    private fun hasDryBreakThisRack(
        match: Match,
        playerId: PlayerId,
    ): Boolean {
     ...
     */
    private fun hasFoulCommittedThisRack(
        match: Match,
        playerId: PlayerId,
    ): Boolean {
        for (event in match.history.asReversed()) {
